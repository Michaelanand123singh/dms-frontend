          ...
          {showPartsRequestModal && selectedJobCard && (
            <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-[9999] p-4">
              <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto p-6 z-[10000] relative">
                ...
                {/* Job Card PART 2 - Parts & Work Items Table */}
                {(() => {
                  // Parse PART 2 data from job card
                  let part2Items: JobCardPart2Item[] = [];
                  
                  // If job card has part2 array, use it directly
                  if (selectedJobCard.part2 && Array.isArray(selectedJobCard.part2) && selectedJobCard.part2.length > 0) {
                    part2Items = selectedJobCard.part2;
                  } else if (selectedJobCard.parts && selectedJobCard.parts.length > 0) {
                    // Convert legacy parts array to PART 2 format
                    part2Items = selectedJobCard.parts.map((part, index) => ({
                      srNo: index + 1,
                      partWarrantyTag: "",
                      partName: part,
                      partCode: part.match(/^([A-Z0-9_-]+)/i)?.[1] || "",
                      qty: 1,
                      amount: 0,
                      technician: "",
                      labourCode: "Auto Select With Part",
                  ...
                  // If we have PART 2 items, display the table
                  if (part2Items.length > 0) {
                    return (
                      <div className="mb-6">
                        <label className="text-sm font-semibold text-gray-700 mb-3 block">
                          Job Card PART 2 - Parts & Work Items
                        </label>
                        ...
                        <div className="mt-3 p-3 bg-gray-50 rounded-lg">
                          <details className="text-xs">
                            ...
                            <pre className="mt-2 p-2 bg-white rounded border border-gray-200 overflow-x-auto text-xs">
                              {JSON.stringify({ part2: jobCardPart2ToJSON(part2Items) }, null, 2)}
                            </pre>
                          </details>
                        </div>
                      </div>
